...
# Escolha do algoritmo a ser utilizado (ex: 'KNN')
...
def main():
  ...
  while cap.isOpened():
    ...
    # Interrompe ao pressionar 'c' ou após 300 frames
    if cv2.waitKey(1) & 0xFF == ord('c') or frame_number >= 300:
      break

  # Calcula o tempo total de processamento
  tick_fim = cv2.getTickCount()
  tempo_processamento = (tick_fim - tick_inicio) / cv2.getTickFrequency()
  print(f"Tempo total de processamento: {tempo_processamento:.2f} segundos")
